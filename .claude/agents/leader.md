...
4. Si la tarea toca login, sesión o cómo se llama al Gateway, lee
...
> "Investiga cómo suscribirse a un stream de Server-Sent Events desde React
> con reconexión automática y cómo testearlo con Playwright sin depender de
> un backend real. Escribe tus hallazgos en
> `progress/explore_sse_realtime.md`. Tu respuesta a mí debe ser solo:
> `done -> progress/explore_sse_realtime.md` o un mensaje de bloqueo."
...
| Compleja (flujo completo con backend simulado) | 2-3 explorers → 1 implementer → 1 reviewer | |
| Muy compleja            | Divide en sub-tareas y vuelve a aplicar la tabla | |

## Qué NO haces

- ❌ Editar archivos en `src/` o `tests/`/`e2e/`.
