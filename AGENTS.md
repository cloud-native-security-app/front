...
| `docs/security-scope.md`     | Límites de autorización/alcance (sesión, tokens, XSS, CSP) | Antes de tocar login, sesión o llamadas al Gateway |
| `CHECKPOINTS.md`             | Criterios objetivos de "estado final correcto"            | Para auto-evaluarte |
| `.claude/agents/`            | Definiciones de subagentes (líder, implementador, revisor) | Si orquestas trabajo |
| `src/`                       | Código de la aplicación (React + TypeScript)              | Para implementar |
| `tests/`                     | Tests unitarios/de componente (`vitest`)                  | Para verificar lógica pura y componentes |
| `e2e/`                       | Tests end-to-end (`playwright`)                            | Para verificar flujos completos en navegador |
...
- **Antes de tocar login, sesión, o cómo se llama al Gateway**, lee
